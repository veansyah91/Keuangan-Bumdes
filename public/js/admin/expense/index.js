...
async function showExpense(){
    try {
        let url = `/api/expense?search=${search}&page=${page}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}`;
                ...
                <div style="width:1%" class="px-2 my-auto">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton${i}">
                            ...
                            ${res.source ? '' : `<li>
                            <a class="dropdown-item" href="/expense/${res.id}/edit">
                                    <div class="row align-items-center justify-conter-start text-success">
                                        <div class="col-2"><i class="bi bi-pencil-square"></i></div>
                                        <div class="col-3">Ubah</div>
                                    </div>
...
function goToPrintExpensePerId(value){
    window.open(`/expense/print-detail/${value.dataset.id}`)
}

function goToPrintExpenses(){
    window.open(`/expense/print?search=${search}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}`)
}
