...
async function showRevenue(){
    try {
        let url = `/api/${business}/revenue?search=${search}&page=${page}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}`;

        let response = await getRevenue(url);
        ...
        if (response.data.length < 1) {
                ...
                <div style="width:10%" class="my-auto">
                    <div class="btn-group dropstart">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton${index}">
                            <li>
                                <button class="dropdown-item" href="#"  data-bs-toggle="modal" data-bs-target="#showDetailModal" onclick="showSingleRevenue(${res.id})" >
            ...
            <div class="d-md-flex d-none justify-content-between border-top border-bottom py-2 px-1 content-data">
                <div style="width:1%" class="px-2 my-auto">
                    <div class="dropdown">
                        ...
                        <ul class="dropdown-menu">
                            ...
                            ${res.source ? '' : `<li>
                            <a class="dropdown-item" href="/${business}/revenue/${res.id}/edit">
                                    <div class="row align-items-center justify-conter-start text-success">
                                        <div class="col-2"><i class="bi bi-pencil-square"></i></div>
                                        <div class="col-3">Ubah</div>
                                    </div>
                ...
                <div style="width:29%" class="px-2 my-auto">${res.description}</div>
...
async function submitDeleteRevenue(){
    try {
        let response = await destroyRevenue(deleteId);
...
async function showSingleRevenue(id){
    try {
        let url = `/api/${business}/revenue/${id}`;
        let res = await getRevenue(url);
...
function goToPrintRevenuePerId(value){
    window.open(`/${business}/revenue/print-detail/${value.dataset.id}`)
}

function goToPrintRevenues(){
    window.open(`/${business}/revenue/print?search=${search}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}`)
}
