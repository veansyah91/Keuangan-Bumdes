...
const updateLabels = Array.from(document.getElementsByClassName('update-label'));
const updateButtons = Array.from(document.getElementsByClassName('update-button'));
const updateButtonLabels = Array.from(document.getElementsByClassName('update-button-label'));
...
async function reloadData(){
    ...
    updateButtons.forEach((button, index) =>{
        ...
        updateButtonLabels[index].innerHTML = `<div class="spinner-border-sm spinner-border" role="status">
                        <span class="visually-hidden">Loading...</span>
                    </div>`;
    })

    try {
        let url = `/api/${business}/over-due/reload`;
...
async function showData(){
    try {
        let url = `/api/${business}/over-due?search=${search}&page=${page}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}${statusQuery}`;
        ...
        updateButtons.forEach((button, index) =>{
            ...
            updateButtonLabels[index].innerHTML = `Perbarui`;
        })
        ...
        if (response.data.length < 1) {
            ...
            let dateUpdate = new Date(response.data[0].updated_at);
            let formatDate = `${dateUpdate.getFullYear()}-${dateUpdate.getMonth() + 1}-${dateUpdate.getDate()}`;
            let now = new Date();
            let formatNow = `${now.getFullYear()}-${now.getMonth() + 1}-${now.getDate()}`;

            if (formatDate === formatNow) {
                updateButtons.forEach(button => {
            ...
            updateLabels.forEach(label => {
                ...
                label.innerHTML = `Pembaruan Terakhir: ${dateReadable(formatDate)}`
            })
            ...
            <div class="d-flex d-md-none justify-content-between border-top border-bottom py-2">
                            ...
                            <li>
                                <button class="dropdown-item" href="#"  data-bs-toggle="modal" data-bs-target="#deleteConfirmationModal" onclick="deleteData(${res.id})" >
                                    <div class="row align-items-center justify-conter-start text-danger">
                                        <div class="col-2"><i class="bi bi-receipt-cutoff"></i></div>
                                        <div class="col-3">Hapus Piutang</div>
                                    </div>
                                </button>
                            </li>
                        </ul>
                ...
                <div style="width:50%" class="my-auto">
                    <div class="font-bold">
                        ${res.account_receivable.contact_name}
                    </div>
                    <div class="">
                        <small>No Ref: ${res.account_receivable.no_ref}</small>
                    </div>
                </div>
                <div style="width:40%" class="my-auto text-end">
                    ...
                    <div class="font-bold">
                        ${dateReadable(res.account_receivable.due_date_temp)}
                    </div>
                    <div>
                        <small>${res.over_due} Hari</small>
                    </div>
                </div>
            ...
            <div class="d-md-flex d-none justify-content-between border-top border-bottom py-2 px-1 content-data">
                ...
                <div style="width:15%" class="px-2 my-auto">${res.account_receivable.no_ref}</div>
                <div style="width:15%" class="px-2 my-auto">${res.account_receivable.contact_name}</div>
                <div style="width:20%" class="px-2 my-auto">${dateReadable(res.account_receivable.due_date_temp)}</div>                 
                <div style="width:20%" class="px-2 my-auto">${res.over_due} Hari</div>                 
...
async function changeDate(value){
    let url = `/api/${business}/no-ref-over-due-recomendation?date=${value.value}`;
...
async function submitDeleteData(){  
    try {
        ...
        let message = `Piutang ${response.no_ref} Berhasil Dihapus`;
