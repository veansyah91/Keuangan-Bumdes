...
async function showStockOpname(){
    try {
        let response = await getStockOpname(search,page, date_from, date_to, thisWeek, thisMonth, thisYear);
                ...
                <div style="width:10%" class="my-auto">
                    <div class="btn-group dropstart">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton${index}">
                            <li>
                                <button class="dropdown-item" href="#"  data-bs-toggle="modal" data-bs-target="#showDetailModal" onclick="showSingleStockOpname(${res.id})" >
                            ...
                            <li>
                                <a class="dropdown-item" href="/${res.business_id}/stock-opname/${res.id}/edit" >
                            ...
                            <li>
                                <button class="dropdown-item" href="#"  data-bs-toggle="modal" data-bs-target="#deleteConfirmationModal" onclick="deleteStockOpname(${res.id})" >
...
function deleteStockOpname(id){
    deleteId = id;
}

async function submitDeleteStockOpname(){
    try {
        let response = await destroyStockOpname(deleteId);
...
async function showSingleStockOpname(id){
    try {
        let res = await getSingleStockOpname(id);
        ...
        res.listInput.map(list => {
            ...
            <div class="row mt-2 text-gray">
                ...
                <div class="col-2 text-end p-2 border border-white">
                    ${list.qty_book + list.qty_balance}
                </div>
                <div class="col-2 text-end p-2 border border-white">
                    ${list.qty_balance}
                </div>
        ...
        document.querySelector('#btn-submit-print-single').setAttribute('href',`/${res.business_id}/stock-opname/print-detail/${res.id}`);
...
function goToPrintStockOpnamePerId(value){
    window.open(`/stock-opname/print-detail/${value.dataset.id}`)
}

function goToPrintStockOpnames(){
    window.open(`/${business}/stock-opname/print?search=${search}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}`)
}
