...
const btnSubmitExpense = document.querySelector('#btn-submit-expense');
...
const setDefaultValue = async () => {
    ...
    let url = `/api/${business}/expense/${pageContent.dataset.id}`;
    let response = await getExpense(url);

    totalInputList = response.ledgers.length - 1;
    totalInput = response.value;

    formData = {
        contact:response.contact,
        from_account: {
            id:response.ledgers[0].account_id,
            name: response.ledgers[0].account_name,
        },
        date: response.date,
    ...
    for (let index = 1; index < response.ledgers.length; index++) {
        formData.listInput = [...formData.listInput, {
            accountId:response.ledgers[index].account_id,
            accountName: response.ledgers[index].account_name,
            total:response.ledgers[index].debit,
        }]
...
const validationInput = () => {
    ...
    if (formData.desc && formData.no_ref && formData.contact && formData.from_account.name) {
        isValidated = true
    ...
    isValidated 
    ? btnSubmitExpense.classList.remove('d-none')
    : btnSubmitExpense.classList.add('d-none')
...
const setValueInputComponent = () => {
    ...
    totalLabel.innerHTML = `Rp.${formatRupiah(formData.value.toString())}`;
...
const componentListInput = (index) => {
    return `<div class="list-input-expense">
        <div class="row justify-content-between py-2">
            ...
            <div class="col-5 text-end">
                <input type="text" class="form-control text-end total-input" inputmode="numeric" autocomplete="off" onclick="this.select()" value="${formatRupiah(formData.listInput[index].total.toString())}" onkeyup="setCurrencyFormat(this)" onchange="changeTotal(this)" data-order="${index}">
            </div>
            <div class="col-1">
...
const componentFromAccountDropdownList = async (value) => {
    let fromAccounts = await getAccounts(value, 1);

    let list = '';
    fromAccounts.data.map(fromAccount => {
        list += `<button type="button" class="list-group-item list-group-item-action" onclick="selectFromAccount(this)" data-name="${fromAccount.name}" data-id="${fromAccount.id}">
        ${fromAccount.name}
    </button>`
    });
...
const showFromAccountDropdown = (value) => {
    contactList.innerHTML = `
            <button type="button" class="list-group-item list-group-item-action text-center">
            <div class="spinner-grow" style="width: .7rem; height: .7rem;" role="status">
                <span class="visually-hidden">Loading...</span>
    ...
    setTimeout(async () => {
        ...
        let list = await componentFromAccountDropdownList(value.value);
        let empty = `<button type="button" class="list-group-item list-group-item-action text-center disabled" disabled>
            No Option
        </button>`;
...
const addListInput = () => {
    ...
    row.className = 'list-input-expense';
...
const deleteRowInput = (value) => {
    ...
    totalInputList--;
    //hapus data pada expenseData
    formData.listInput.splice(parseInt(order), 1);

    // hapus element 
    document.getElementsByClassName('list-input-expense')[parseInt(order)].remove();
...
async function submitExpense(){
    ...
    try {
        ...
        let result = await putExpense(formData, pageContent.dataset.id);
