...
async function showAccountPayable(){
    try {
        let response = await getAccountPayable(search,page, date_from, date_to, thisWeek, thisMonth, thisYear);
        ...
        if (response.data.length < 1) {
            ...
            total += res.value;
            let sisa = parseInt(res.account_payables_sum_credit) - parseInt(res.account_payables_sum_debit);
            list += `
                ...
                <div style="width:10%" class="my-auto">
                    <div class="btn-group dropstart">
                        ...
                        <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton${index}">
                            <li>
                                <button class="dropdown-item" href="#"  data-bs-toggle="modal" data-bs-target="#showDetailModal" onclick="showSingleAccountPayable(${res.id})" >
                            ...
                            <li>
                                <a class="dropdown-item" href="/${res.business_id}/account-payable/${res.id}/edit" >
                            ...
                            <li>
                                <button class="dropdown-item" href="#"  data-bs-toggle="modal" data-bs-target="#deleteConfirmationModal" onclick="deleteAccountPayable(${res.id})" >
            ...
            <div class="d-md-flex d-none justify-content-between border-top border-bottom py-2 px-1 content-data">
                ...
                <div style="width:20%" class="px-2 my-auto text-end">${formatRupiah(res.account_payables_sum_credit)}</div>
                <div style="width:20%" class="px-2 my-auto text-end">${formatRupiah(res.account_payables_sum_debit)}</div>
...
function deleteAccountPayable(id){
    deleteId = id;
}

async function submitDeleteAccountPayable(){
    try {
        let response = await destroyAccountPayable(deleteId);
...
async function showSingleAccountPayable(id){
    try {
        let res = await getSingleAccountPayable(id);
        ...
        let list = '';
        res.purchase_goods.map(invoice => {
            list += `
            <div class="row justify-content-between mt-5">
                <div class="col-4">
                    <span class="fw-bold">Faktur Pembelian: </span>${invoice.no_ref}
                </div>
                <div class="col-4">
                <span class="fw-bold">Tanggal Pembelian: </span>${dateReadable(invoice.date)}
                </div>
        ...
        detailLists.map((detail, index) => {
            let invoices = res.purchase_goods[index];
            let list = '';

            let total = 0;
            let debits = 0;
            let credits = 0;
            if (invoices.account_payables.length > 0) {
                invoices.account_payables.map(accountReceivable => {
        ...
        // document.querySelector('#btn-submit-print-single').setAttribute('href',`/${res.business_id}/account-payable/${res.id}/print-detail`);
...
function goToPrintAccountPayables(){
    window.open(`/${business}/account-payable/print?search=${search}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}`)
}
