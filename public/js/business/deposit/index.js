...
async function showData(){
    try {
        let url = `/api/${business}/deposit?search=${search}&page=${page}&date_from=${date_from}&date_to=${date_to}&this_week=${thisWeek}&this_month=${thisMonth}&this_year=${thisYear}`;
...
async function changeDate(value){
    let url = `/api/${business}/no-ref-deposit-recomendation?date=${value.value}`;
...
const addData = async () => {
    ...
    let url = `/api/${business}/no-ref-deposit-recomendation?date=${formData.date}`;
...
const editData = async (id) => {
    ...
    let url = `/api/${business}/deposit/${id}`;
...
async function showSingleContact(id){
    try {
        ...
        document.querySelector('#btn-submit-print-single').setAttribute('href', `/${business}/deposit/${id}/print-detail`);
