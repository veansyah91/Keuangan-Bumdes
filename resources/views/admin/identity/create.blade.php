...
@section('admin')
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom col-12 col-lg-6">
        <h1 class="h2">Masukkan Data Desa</h1>
        <a href="{{ route('identity.index') }}" class="btn btn-secondary">kembali</a>
    </div>

    <div class="row">
        <div class="col-12 col-lg-6">
            <form method="post" enctype="multipart/form-data" action="{{ route('identity.store') }}">
                @csrf
                <div class="mb-3 row">
                    <label for="kepala_desa" class="col-sm-3 col-form-label">Kepala Desa</label>
                    <div class="col-sm-9">
                        <input type="text" class="form-control @error('kepala_desa') is-invalid @enderror" id="kepala_desa" name="kepala_desa" value="{{ old('kepala_desa') }}">
                    </div>
                </div>
                <div class="mb-3 row">
