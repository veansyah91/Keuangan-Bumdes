...
@section('content')
    <div class="d-flex col-12">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('business.stock.index', $business->id) }}">Produk</a></li>
    ...
    <div class="row justify-content-center">
        <div class="col-md-3 col-12">
            <div class="card" id="input-card">
                ...
                <form action="{{ route('business.stock.store', $business->id) }}" method="post" class="form-input">
                    @csrf
                    <div class="card-body">
                        <x-product.form 
                            ...
                            @if ($products->isNotEmpty())
                                @foreach ($products as $product)
                                    <tr>
                                        ...
                                        <td>
                                            @if (!IncomingItemHelper::getData($product->stock->incomingitem_id))
                                                <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="deleteConfirmation({{ $business->id }}, {{ $product->stock->id }}, 'create')">hapus</button>
                                                <a href="#edit-card" class="btn btn-sm btn-secondary" onclick="editConfirmation({{ $business->id }}, {{ $product->stock->id }})">ubah</a>
                                            @endif
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        const editConfirmation = (business_id, stock_id) => {
            ...
            formInput[1].setAttribute('action', `/${business_id}/stock/${stock_id}`);
        ...
        const deleteConfirmation = (businessId, stockId, type) => {
