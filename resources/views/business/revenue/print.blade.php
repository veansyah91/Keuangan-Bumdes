    ...
    <div class="p-4">
        ...
        <div class="row justify-content-center mb-3">
            <div class="col-12 text-center">
                <h1 class="text-gray">Laporan Pendapatan</h1>
            </div>
        </div>
            ...
            <table class="table mt-5">
                ...
                <tbody>
                    @foreach ($revenues as $revenue)
                        <tr>
                            <td style="width: 15%">{{ $carbon::parse($revenue->date)->isoformat('MMM, D Y') }}</td>
                            <td style="width: 20%">{{ $revenue->no_ref }}</td>
                            <td style="width: 20%">{{ $revenue->description }}</td>
                            <td style="width: 25%">{{ $revenue->detail }}</td>
                            <td class="text-end" style="width: 20%">{{ number_format($revenue->value, 0, '', '.')}}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
