...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header fs-4 fw-bold">{{ __('Kategori') }}</div>
                                    ...
                                    @if ($categories->isNotEmpty())
                                        @foreach ($categories as $category)
                                            <tr>
                                                <td>{{ $category->nama }}</td>
                                                <td>
                                                    <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="deleteConfirmation({{ $business->id }}, {{ $category->id }})">hapus</button>
                                                </td>
                                            </tr>
                                        @endforeach                                    
                                    @else
    ...
    <!-- Input Modal -->
    <form action="{{ route('business.category.store', $business->id) }}" method="post" id="form-input">
        @csrf
...
@section('script')
    <script type="text/javascript">
        ...
        saveButton.addEventListener('click', function()
        {
            ...
            // validasi
            axios.post(`/api/${businessId.value}/category`, data)
            .then((res) => {
                console.log(res);
                formInput.submit()
            })
        ...
        const deleteConfirmation = (businessId, id) => {
            ...
            deleteConfirmationForm.setAttribute('action', `/${businessId}/category/${id}`);
        }
