...
@section('content')
    <div class="row justify-content-center">
        <div class="col-md-8 col-12">
            <div class="card">
                <div class="card-header fs-4 fw-bold">{{ __('Pelanggan') }}</div>
                                    ...
                                    @if ($customers->isNotEmpty())
                                        @foreach ($customers as $customer)
                                            <tr>
                                                ...
                                                <td>
                                                    <button class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteModal" onclick="deleteConfirmation({{ $business->id }}, {{ $customer->id }})">hapus</button>
                                                    <button class="btn btn-sm btn-secondary" data-bs-toggle="modal" data-bs-target="#editModal" onclick="editConfirmation({{ $business->id }}, {{ $customer->id }})">ubah</button>
                                                </td>
                                            </tr>
                                        @endforeach                                    
                                    @else
    ...
    <!-- Input Modal -->
    <form action="{{ route('business.customer.store', $business->id) }}" method="post" id="form-input">
        @csrf
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="col-12 mt-2">
                            ...
                            <div class="mb-3">
                                <label for="no_hp" class="form-label">No HP Pelanggan</label>
...
@section('script')
    <script type="text/javascript">
        ...
        saveButton.addEventListener('click', function()
        {
            ...
            // validasi
            axios.post(`/api/${businessId.value}/customer`, data)
            .then((res) => {
                console.log(res);
                formInput.submit()
            })
        ...
        const editConfirmation = (businessId, id) => {
            formEdit.setAttribute('action', `/${businessId}/customer/${id}`);

            axios.get(`/api/${businessId}/customer/${id}`)
            .then((res) => {
                const nama = document.getElementById('nama-edit');
                const alamat = document.getElementById('alamat-edit');
                const no_hp = document.getElementById('no-hp-edit');
        ...
        const deleteConfirmation = (businessId, id) => {
            ...
            deleteConfirmationForm.setAttribute('action', `/${businessId}/customer/${id}`);
        }
